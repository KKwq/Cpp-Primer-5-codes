...
int main() {
    ...
    /* this loop uses a range for statement, which is a C++ 11 feature
            ...
            for (const auto &row : ia) // for every element in the outer array
                    for (auto col : row)   // for every element in the inner
       array cout << col << endl; // print the element's value cout << ia[0][0]
       << ' ' << ia[2][3] << endl; // prints 0 11
    */
    ...
    for (int(*p)[4] = ia; p != ia + rowCnt; ++p) {
        ...
        for (int *q = *p; q != *p + colCnt; ++q) cout << *q << ' ';
        cout << endl;
    }

    /* begin and end functions are a c++ 11 feature
            // 3. using pointers and a traditional for loop
            //    with the library begin and end functions to manage the
       pointers for (int *p = begin(ia); p != end(ia); ++p) {
                    // q points to the first element in an inner array
                    for (int *q = begin(*p); q != end(*p); ++q)
                            cout << *q << ' ';  // prints the int value to which
       q points cout << endl;
            }
    */
    ...
    /* these loops use a range for statement, which is a C++ 11 feature
            ...
            for (auto &row : alt_ia)    // for every element in the outer array
                    for (auto &col : row) { // for every element in the inner
       array col = cnt;          // give this element the next value
                            ++cnt;              // increment cnt
                    }
            // now print the value of the array
            for (const auto &row : alt_ia) // for every element in the outer
       array for (auto col : row)     // for every element in the inner array
                            cout << col << endl;
    */
